...
exports.obtenerReuniones = async (request, response) => {
  try {
    const { userId, corporacionId } = request.query;
...
exports.crearReunion = async (request, response) => {
  try {
    ...
    if (idUsuarios.length === 0) {
      return response
        .status(400)
        .json({ error: "La lista de asistentes no puede estar vacía" });
    }
    const nuevaReunion = new Reunion({
      ...
      corporacion: reunion.corporacion,
    });
    ...
    await Corporacion.findByIdAndUpdate(reunion.corporacion, {
      $push: { reuniones: reunionGuardada._id },
    });
    response.status(201).json({
      mensaje: "Reunion creada y usuarios actualizados",
      reunion: reunionGuardada,
      usuariosAfectados: idUsuarios,
    });
  } catch (error) {
    response
      .status(500)
      .json({ error: "Error al crear la reunión", mensaje: error.message });
  }
};
...
exports.obtenerUsuariosReunion = async (request, response) => {
  try {
    ...
    response
      .status(500)
      .json({ error: "Error al obtener usuarios de la reunion por su id" });
  }
};
...
exports.actualizarReunion = async (request, response) => {
  try {
    ...
    const Actualizada = await Reunion.findByIdAndUpdate(id, reunion, {
      new: true,
    });
    ...
    if (reunion.asistentes) {
      ...
      await Promise.all([
        //Para ejecutar dos operaciones de acceso a la base de datos a la vez
