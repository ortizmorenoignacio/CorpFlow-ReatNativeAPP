...
exports.crearCarpeta = async (request, response) => {
  try {
    ...
    if (carpetaPadre) {
      ...
      await Promise.all([
        Carpeta.findByIdAndUpdate(carpetaPadre, {
          $push: { carpetasHijas: savedFolder._id },
        }),
        Corporacion.findByIdAndUpdate(parentFolderDoc.corporacion, {
    ...
    //Caso 2: Es carpeta Raiz
      ...
      if (!corporacion) {
        return res
          ...
          .json({
            error:
              "Para crear una carpeta raíz, se requiere el campo 'corporacion'",
          });
      }
      ...
      if (!corpExiste) {
        return res
          .status(404)
          .json({ error: "La corporación especificada no existe" });
      }
      ...
      await Corporacion.findByIdAndUpdate(savedFolder.corporacion, {
        $push: { carpetas: savedFolder._id },
      });
      return response.status(201).json({
    ...
    response
      .status(500)
      .json({ error: "Error al crear la carpeta", detalle: error.message });
  }
};
...
exports.borrarCarpeta = async (request, response) => {
  try {
    ...
    if (!carpeta) {
      return response
        .status(404)
        .json({ error: "No existe la carpeta deseada con ese id" });
    }

    await eliminarContenido(id);

    await Promise.all([
      Corporacion.findByIdAndUpdate(carpeta.corporacion, {
        $pull: { carpetas: id },
      }),
      carpeta.carpetaPadre
        ? carpeta.findByIdAndUpdate(carpeta.carpetaPadre, {
            $pull: { carpetasHijas: id },
          })
        : null,
    ]);
    ...
    response
      .status(500)
      .json({ error: "Error al borrar", detalle: error.message });
  }
};
...
exports.actualizarCarpeta = async (request, response) => {
  try {
    ...
    const Actualizada = await Carpeta.findByIdAndUpdate(id, carpeta, {
      new: true,
    });
...
exports.obtenerCarpetasHijas = async (request, response) => {
  try {
    ...
    response
      .status(500)
      .json({ error: "Error al obtener subcarpetas de una carpeta por su id" });
  }
};
...
exports.obtenerDocumentos = async (request, response) => {
  try {
    ...
    response
      .status(500)
      .json({ error: "Error al obtener documentos de una carpeta por su id" });
  }
};
