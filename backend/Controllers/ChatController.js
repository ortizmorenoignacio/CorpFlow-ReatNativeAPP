...
exports.crearChat = async (request,response) => {
    try {
        ...
        if(corporacionExiste){
            return response.status(400).json({error: "Ya existe un chat para esa corporacion"})
        }
        ...
        response.status(500).json({error: "Error al crear chat", detalle: error.message})
    }
}
...
exports.actualizarChat = async (request,response) => {
    try {
        ...
        const Actualizada = await Chat.findByIdAndUpdate(
            id,chat,{new:true}
        )
        ...
        response.status(500).json({ error: "Error al actualizar el chat", detalle: error.message });
    }
}
    ...
    try {
        ...
        if (chat){
            if (mensajes.length !=0){
                ...
                response.json({Info: "El usuario no tiene mensajes asignados"})
            }
        }
    } catch (error) {
        response.status(500).json({error:"Error al obtener mensajes de un chat por su id"})
    }
}