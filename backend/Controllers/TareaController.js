...
exports.crearTarea = async (request, response) => {
  try {
    ...
    await Corporacion.findByIdAndUpdate(tarea.corporacion, {
      $push: { tareas: savedTarea._id },
    });
...
exports.actualizarTarea = async (request, response) => {
  try {
    ...
    const Actualizada = await Tarea.findByIdAndUpdate(id, tarea, { new: true });

    if (tarea.usuario) {
      const newUser = tarea.usuario;

      await Promise.all([
        Usuario.findByIdAndUpdate(usuarioOld, { $pull: { tareas: id } }),
        Usuario.findByIdAndUpdate(newUser, { $push: { tareas: id } }),
      ]);
    }
