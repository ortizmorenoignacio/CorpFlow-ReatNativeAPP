...
exports.obtenerReunionesUsuario = async (request,response) =>{
    try{
        ...
        response.status(500).json({error:"Error al obtener reuniones de un usuario por su id"})
    }
}
...
exports.crearUsuario = async (request,response) =>{
    ...
    try{
        ...
        if(!usuario.nombre || !usuario.correo || !usuario.telefono || !usuario.fotoPerfil ||    !usuario.fechaNacimiento || !usuario.contraseña || !usuario.dni || !usuario.genero
        ){
            return response.status(400).json({error: "require fields are missing"})
        }

        if (usuarioExiste){
            return response. status(400).json({error: "Ya existe un usuario con ese dni"})
        }
        ...
        response.status(500).json({error: "Error al crear usuario", detalle: error.message})
    }
}
...
exports.actualizarUsuario = async (request,response) => {
    try {
        ...
        await Usuario.findByIdAndUpdate(id, newUserInfo, {new: true}).then(result => {
